...
export const buildAgentPrompt = (payload: AgentRequest): string =>
  [
    ...
    "",
    "STEP 1 - Relevance. Craigslist search is loose. Decide whether this",
    "listing IS the thing the buyer asked for. Accessories, different product",
    "lines and merely similar brands do NOT count. If it is not the item, set",
    "matchesQuery=false, isGoodDeal=false, score=0 and say what it actually is.",
    "A cheap wrong thing is not a deal.",
    "",
    "STEP 2 - Value, only if it IS the right item. Price alone is not value.",
    "Weigh all of these and say which ones drove your verdict:",
    ...
    "  - Condition: sealed or new-in-box, like new, used, for parts, or not",
    "    working. 'For parts' is rarely a deal for a buyer who wants a",
    "    working machine.",
    "  - Age and remaining useful life, including whether the model still",
    "    receives OS updates.",
    "  - What is included: keyboard, mouse, original box, warranty or",
    "    AppleCare, bundled extras.",
    ...
    "",
    "The `comparables` list is the market for THIS watch. Compare against the",
    "entries closest in generation and configuration, not against the median,",
    "which mixes generations and is therefore a weak signal on its own. Say so",
    "when the spread makes the median unreliable.",
    "",
    "priceVsMedian = (price - baseline.median) / baseline.median, or 0 when",
    "there is no baseline. Report it even when you did not weight it heavily.",
    "",
    "If baseline is null you have no market history: judge from targetPrice",
    "and your own knowledge of what this configuration is worth used. Be",
    "conservative.",
    ...
    "",
    "score reflects overall value for this buyer, not the discount. A pristine",
    "current-generation unit at a fair price can outscore an ancient one that",
    "is nominally cheaper.",
    "",
    "Input:",
    JSON.stringify(payload, null, 2),
    "",
    'Reply with ONLY this JSON object and no prose: {"matchesQuery": boolean, "isGoodDeal": boolean, "score": 0-100, "reasoning": "one or two sentences naming the deciding factors", "priceVsMedian": number}',
  ].join("\n");
